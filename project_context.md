...
- Погода і час доби транслюються всім клієнтам через UpdateWeather
  ...
  - weatherNextChangeAt оновлюється ВСЕРЕДИНІ rollWeather() перед
    трансляцією (як і phaseNextChangeAt у циклі дня/ночі) — раніше
    оновлювалась до task.wait(), тому трансляція в момент зміни
    рахувала вже застарілу мітку (~0с), і клієнтський таймер погоди
    "вмирав" на 0:00 назавжди після першої ж зміни (виправлено)
...
- Офлайн пасивний дохід (F3) — одноразовий лямп-сум при вході з тостом:
  ...
  (1+бонус префікса) × (1+бонус колекції: +30% якщо в ЦІЙ зоні є хоч 1
  кожного виду; +100% ЗАМІНЮЄ +30% в ОБОХ зонах одночасно, але тільки
  якщо ввесь музей — обидві зони — зібраний по максимуму кожного виду)
- Погода в момент лову НЕ впливає на дохід/ціну риби взагалі (рішення
  2026-07-02, документ E2/B2 оновлено — початковий "+50% за Грозу/Туман"
  відхилено)
...
- Спільні геометричні константи ландшафту (HUB_ORIGIN, MAX_SLOTS=8,
  ISLAND_SPACING по Z, розмір острова, зсув причалу/океану/зон) +
  функції islandOrigin(slot), pierZ(), oceanStartZ(), zone1CenterZ(),
  zone2CenterZ() — щоб HubBuilder і WorldBuilder рахували ту саму
  систему координат і не розсинхронізувались
- РІШЕННЯ (2026-07-02): відстані навмисно СТИСНУТІ до "тестового"
  масштабу (причал/океан/зони можна дійти пішки за ~30-60с), бо
  початкові значення рахували причал від краю ВСІХ 8 слотів (навіть
  не зайнятих), що давало ~3800 studs пішого шляху без човна —
  нереально для тестування. Коли з'явиться човен/паті-система,
  значення можна збільшити назад, змінивши лише цей файл
...
11. Погода/час у момент лову НЕ впливають на дохід музею чи ціну риби
    взагалі — документ початково мав "+50% за Грозу/Туман" до доходу
    музею (E2/B2), відхилено назавжди
